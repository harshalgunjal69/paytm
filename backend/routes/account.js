...
router.get('/balance', authMiddleware, async function (req, res) {
    ...
    try {
        ...
        if (accountBalanceFound) {
            console.log(
                'its returning the balance:' + accountBalanceFound.balance
            );
            return res
                .status(200)
                .json({ balance: accountBalanceFound.balance });
        } else {
            return res
                .status(404)
                .json({ message: 'Couldnt find the account' });
        }
    } catch (error) {
        res.send(500).json({
            msg: 'getting error while checking balance of the user.',
        });
    }
});
...
router.post('/transfer', authMiddleware, async function (req, res) {
    ...
    try {
        ...
        return res
            .status(500)
            .json({ error: 'error occurred during transfer.' });
    }
});
